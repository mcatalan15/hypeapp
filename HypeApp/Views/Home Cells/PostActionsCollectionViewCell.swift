...
protocol PostActionsCollectionViewCellDelegate: AnyObject {
    ...
    func postActionsCollectionViewCellDidTapBuy(_ cell: PostActionsCollectionViewCell, index: Int)
}
...
class PostActionsCollectionViewCell: UICollectionViewCell {
    ...
    //Var para predefinir like como no ejecutado
    ...
    @objc func didTapBuy() {
        delegate?.postActionsCollectionViewCellDidTapBuy(self, index: index)
    }


    
    //Salida por Screen, posicionamiento botones
    override func layoutSubviews() {
        ...
        buyButton.frame = CGRect(x: shareButton.right+180, y: (contentView.height-size), width: size, height: size)
